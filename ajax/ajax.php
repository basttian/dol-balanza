...
/***********************************************************************************************************************************************/
/***********************************************************************************************************************************************/
//IMPORTANTE!!! ///// *********** El TAKEPOS debe tener desactivado la opcion ***Agrupar mismas líneas de producto****
  	$arrcode = desglocode($term);
	//[20]Bandera o Flag del codigo generado por balanza
	//Los cinco primeros son del codigo del producto
	//Los cinco siguientes del peso o precio del producto
	//El ultimo codigo es el check digit
	if($arrcode[0]==20 || $arrcode[0]==21 || $arrcode[0]==22 || $arrcode[0]==23 || $arrcode[0]==24 || $arrcode[0]==25 ){//Cinco codigos de balanza pueden usar solo el 20
		require_once DOL_DOCUMENT_ROOT.'/product/class/product.class.php';
		$prod = new Product($db);
		$res = $prod->fetch('','','',$arrcode[0].$arrcode[1]);
		if($res){
			$rows[] = array(
			'rowid' => $prod->id,
			'label'   => $prod->label,
			'barcode' => $arrcode[0].$arrcode[1],	
			'price'   => ((int)$arrcode[2] * $prod->price) / $prod->weight ,
			'qtycode' => (int)$arrcode[2] / $prod->weight ,
			'object'  => 'balancode',
			);
			...
			for ($i = 0; $i < 10; $i++) {
				$prod->fetch('','','',$arrcode[0].$arrcode[1].'00000'.$i);
				if ($i == $arrcode[3]) {
					$rows[] = array(
							'rowid' => $prod->id,
							'label'   => $prod->label,
							'barcode' => $arrcode[0].$arrcode[1].'00000'.$i,
							'price'   => ((int)$arrcode[2] * $prod->price) / $prod->weight ,
							'qtycode' => (int)$arrcode[2] / $prod->weight ,
							'object'  => 'balancode',
					);
...
/***********************************************************************************************************************************************/
/*****************************************************************************************************************************************************/
function desglocode($code){
	//Estos valores dependen de la configuracion de la balanza
	$codeo = array();
	$varflagconf = 2;
	$codProdconf = 5;
	$codPriceconf = 5;
	...
	//La suma no debe pasar los 13 caracteres
	$codigocant = $varflagconf + $codProdconf + $codPriceconf + $checkDigitconf;
	if ($codigocant==13) {
		for($i = 0; $i < $varflagconf; $i++) {
			$flag.= $code[$i];
		}
		for($i = $varflagconf; $i < ($varflagconf+$codProdconf); $i++) {
			$codProd.= $code[$i];
		}
		for($i = ($varflagconf+$codProdconf); $i < ($varflagconf+$codProdconf+$codPriceconf); $i++) {
			$codPrice.= $code[$i];
		}
		for($i = ($varflagconf+$codProdconf+$codPriceconf); $i < ($varflagconf+$codProdconf+$codPriceconf+$checkDigitconf); $i++) {
			$checkDigit.= $code[$i];
		}
		//Retornamos el codigo desglosado en un arreglo de 3 posisciones
		$codeo = array($flag,$codProd,$codPrice,$checkDigit);
		return $codeo;
	}
}
/***********************************************************************************************************************************************/
